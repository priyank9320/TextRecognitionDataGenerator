...
class GeneratorFromWikipedia:
    ...
    def __init__(
        ...
        max_sentence_length: int = 4, ## max number of words in a sentence
        fonts: List[str] = [],
        language: str = "en",
        size: int = 32,
        skewing_angle: int = 0,
        ...
        self.generator = GeneratorFromStrings(
            create_strings_from_wikipedia(
                self.minimum_length, self.batch_size, self.language, max_sentence_length=self.max_sentence_length
            ),
            count,
    ...
    def next(self):
        if self.generator.generated_count >= self.steps_until_regeneration:
            self.generator.strings = create_strings_from_wikipedia(
                self.minimum_length, self.batch_size, self.language, self.max_sentence_length
            )
            self.steps_until_regeneration += self.batch_size
